...
USE_MPS_EXPERIMENTAL=False
USE_MY_MPS_OPTIMIZATION=True
...
#old function
def xx_center_down_sample_3d(x, kernel_size):
    # pt, ph, pw = kernel_size
    # cp = (pt * ph * pw) // 2
...
def center_down_sample_3d(x, kernel_size):
    # on mac we try to use Metal and not fall back to CPU
    if platform.system() == 'Darwin' and torch.backends.mps.is_available() and USE_MY_MPS_OPTIMIZATION:
        x_avg=None
        #trying two metal compliant calcs
        usepath1=False
        if usepath1:
            # Alternative MPS implementation without einops
            B, C, T, H, W = x.shape
            kt, kh, kw = kernel_size
            
            # Reshape to extract blocks
            x = x.view(B, C, T//kt, kt, H//kh, kh, W//kw, kw)
            x = x.permute(0, 1, 2, 4, 6, 3, 5, 7)  # [B, C, T', H', W', kt, kh, kw]
            x = x.reshape(-1, kt*kh*kw)  # flatten the kernel dimensions
            
            # Select center element
            center_idx = (kt * kh * kw) // 2
            x = x[:, center_idx]
            
            # Reshape back
            x_avg= x.view(B, C, T//kt, H//kh, W//kw)
        else:
            # Manual average pooling alternative using rearrange
            pt, ph, pw = kernel_size
            x_unfolded = einops.rearrange(
                x, 'b c (t pt) (h ph) (w pw) -> b c t h w pt ph pw',
                pt=pt, ph=ph, pw=pw
            )
            x_avg = x_unfolded.mean(dim=(-1, -2, -3))  # Average over pt, ph, pw
