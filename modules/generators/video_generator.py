...
from diffusers_helper.memory import DynamicSwapInstaller, synchronize_gpu, empty_cache
...
class VideoModelGenerator(BaseModelGenerator):
    ...
    def __init__(self, **kwargs):
        ...
        self.model_path =  resolved_paths["PATH_MODEL_FramePackI2V_HY"]   # Same as Original
    ...
    def video_encode(self, video_path, resolution, no_resize=False, vae_batch_size=16, device=None, input_files_dir=None):
        ...
        Args:
            ...
            device: Device for computation (e.g"c u d a").
        ...
        # Check xGPU availability and fallback to CPU if needed
        ...
        try:
            ...
            if NEW_CODE:
                ...
                stream_for_fps = container.streams.video[0]
                fps = float(stream_for_fps.average_rate)
                # Read frames
                frames = []
                ...
                print("Alternative code execution for video processing.")
                # Load video and get FPS
                container = av.open(video_path)

                # Get video stream (usually the first one)
                video_stream = next(s for s in container.streams if s.type == 'video')
                fps = float(video_stream.average_rate)  # Get input video FPS
                num_real_frames = video_stream.frames

                # Handle cases where frame count isn't available
                if num_real_frames == 0:
                    num_real_frames = int(container.duration * video_stream.time_base * fps)
                ...
                frames = []
                for frame in container.decode(video=0):
                    if len(frames) >= num_real_frames:
                        break
                    # Convert to numpy array (HWC, RGB format)
                    frames.append(frame.to_ndarray(format='rgb24'))

                container.close()
                frames = np.array(frames)  # Shape: (num_real_frames, height, width, channels)                
            ...
            if device == "cuda" or device =="mps":
                ...
                empty_cache()
                #print("VAE moved back to CPU, GPU cache cleared")
