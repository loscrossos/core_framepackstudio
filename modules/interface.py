...
fallbacks = {
    "PATH_MODEL_Output": settings_path_outputs,
}
...
WATCH_FOLDER = resolved_paths["PATH_MODEL_Output"]  
POLL_INTERVAL = 4  # Seconds between checks

# Supported video extensions
VIDEO_EXTENSIONS = ('.mp4', '.mov', '.avi', '.mkv', '.webm')
# --- Initialization time reference ---
startup_reference_time = time.time()

def find_latest_video_after_reference():
    """Return the latest video file modified after startup_reference_time."""
    try:
        files = [
        ...
        files_after_ref = [f for f in files if os.path.getmtime(f) > startup_reference_time]
        if not files_after_ref:
            return None
        return max(files_after_ref, key=os.path.getmtime)
    except Exception as e:
        print(f"[ERROR] Folder scan failed: {e}")
        return None
#ENDPREFIX#######################################################################
...
def create_interface(
    ...
    block = gr.Blocks(css=css, title="FramePackStudio_core", theme=current_theme).queue()
    ...
    with block:
        ...
        with gr.Row(elem_id="fixed-toolbar"):
            with gr.Column(scale=0, min_width=400):
                ...
                <div style="display: flex; align-items: center;">
                    <h1 class='toolbar-title'>FramePackStudio_core</h1>
                </div>
                """)
                ...
                with gr.Row():
                    with gr.Column(scale=2):
                        ...
                        with gr.Accordion("Generation Parameters", open=True):
                            with gr.Row():
                                steps = gr.Slider(label=steps_label, info=steps_info, minimum=1, maximum=100, value=init_if_steps, step=1)
                                total_second_length = gr.Slider(label="Video Length (Seconds)", minimum=1, maximum=120, value=init_if_video_length, step=0.1)
                            with gr.Group():
                                with gr.Row("Resolution"):
                                    resolutionW = gr.Slider(
                                        label=resolution_label, minimum=128, maximum=768, value=init_if_resolution, step=32, 
                                        info=resolution_info
                                    )
                                    resolutionH = gr.Slider(
                                        label="Height", minimum=128, maximum=768, value=init_if_resolution, step=32, 
                                        info=resolution_info
                                    )
                                    fps = gr.Slider(
                            ...
                            def on_resolution_change(img, resolutionW, resolutionH):
                                out_bucket_resH, out_bucket_resW = [init_if_resolution, init_if_resolution]
                                if img is not None:
                                    H, W, _ = img.shape
                        ...
                        with gr.Accordion("Advanced Parameters", open=False):
                            latent_window_size = gr.Slider(label="Latent Window Size", minimum=1, maximum=33, value=init_if_latentwindowsize, step=1, visible=True, info='Change at your own risk, very experimental')  # Should not change
                    ...
                    with gr.Column():
                        ...
                        result_video = gr.Video(label="Finished Frame display", autoplay=True, show_share_button=False, height=256, loop=True)
                ...
                with gr.Row():
                    with gr.Column():
                        ...
                        gpu_memory_preservation = gr.Slider(
                            label=gpu_preserve_label,
                            minimum=1,
                            maximum=128,
                            step=0.1,
                            value=settings.get("gpu_memory_preservation", init_if_uram),
                            info=gpu_preserve_info
                        )
                        mp4_crf = gr.Slider(
        ...
        def process_with_queue_update(model_type, *args):
            ...
            result = process_fn(model_type, input_data, actual_end_frame_image_for_backend, actual_end_frame_strength_for_backend, prompt_text, n_prompt, seed_value, total_second_length, # Pass original prompt_text string
                            ...
                            use_teacache, teacache_num_steps, teacache_rel_l1_thresh, blend_sections, latent_type, clean_up_videos, selected_loras, resolutionW, resolutionH, fps, *lora_args)
        ...
        #MacOS Workaround
        if sys.platform == "darwin":
            print("MacOS Detected: video preview based on file")
            start_button.click(
                fn=update_video,
                outputs=[result_video]
            )
            test= gr.Video(label="dummy", autoplay=False, show_share_button=False, height=256, loop=False, visible=False)
            ...
            start_button.click(
                ...
                outputs=[test, current_job_id, preview_image, progress_desc, progress_bar, start_button, end_button, queue_status, seed]
            )
            ...
            current_job_id.change(
                ...
                outputs=[test, current_job_id, preview_image, progress_desc, progress_bar, start_button, end_button]
            )
        else:           
            # --- Connect Buttons ---
            start_button.click(
...
def update_video():
    last_seen = None
    print("LCX: MAcOSwatcher started!")
    while True:
        latest = find_latest_video_after_reference()
        if latest and latest != last_seen:
            last_seen = latest
            print(f"[Watcher] New video detected: {latest}")
            yield latest
            #call update
