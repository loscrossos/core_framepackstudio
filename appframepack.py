...
MODEL_CONFIG_FILE="modelconfig.txt"
from pathlib import Path
def ensure_model_paths_file(file_path="modelpaths.txt"):
    """
    Checks if the specified file exists. If not, creates it with default model path content.
    """
    path = Path(file_path)
    content = """\
...
"""
    if not path.exists():
        ...
        print(f"First Start. Created: {file_path}")

ensure_model_paths_file(MODEL_CONFIG_FILE)

import os
from dotenv import load_dotenv
# Load pahts from the current directory
load_dotenv(MODEL_CONFIG_FILE)
# Define variable names and their fallback values
local_hf_home= os.path.abspath(os.path.realpath(os.path.join(os.path.dirname(__file__), './models/hf_download')))
fallbacks = {
...
for var, fallback in fallbacks.items():
    path = os.getenv(var)
    if path:
        local_paths[var] = os.path.abspath(path)
...
if args.checkmodels: 
    ...
    test_paths_hf = [
       f"{local_paths["HF_HOME"]}{os.sep}hub{os.sep}models--hunyuanvideo-community--HunyuanVideo{os.sep}snapshots" , 
       f"{local_paths["HF_HOME"]}{os.sep}hub{os.sep}models--lllyasviel--flux_redux_bfl{os.sep}snapshots" , 
       f"{local_paths["HF_HOME"]}{os.sep}hub{os.sep}models--lllyasviel--FramePack_F1_I2V_HY_20250503{os.sep}snapshots" , 
       f"{local_paths["HF_HOME"]}{os.sep}hub{os.sep}models--lllyasviel--FramePackI2V_HY{os.sep}snapshots" ,         
        ]
    
    test_paths_dir = [
        f"{local_paths[  "PATH_MODEL_HunyuanVideo"]}{os.sep}config.json",
        f"{local_paths[  "PATH_MODEL_flux_redux_bfl"]}{os.sep}model_index.json",
        f"{local_paths[  "PATH_MODEL_FramePack_F1_I2V_HY_20250503"]}{os.sep}config.json",
        f"{local_paths[  "PATH_MODEL_FramePackI2V_HY"]}{os.sep}config.json",
  
        ]
    print("framepackStudio_core: checking model accessibility")
    summary_hf, all_exist_hf, none_exist_hf, path_details_hf = count_existing_paths(test_paths_hf)

    print(f"\nSearching Group1: HF_HOME:")
    for line in path_details_hf:
        print_line= remove_suffix(line, "snapshots")
        print(print_line)

    summary_dir, all_exist_dir, none_exist_dir, path_details_dir = count_existing_paths(test_paths_dir)
    print("Searching Group2: Manual:")
    ...
    if none_exist_dir and none_exist_hf:
        ...
        print(f"Current defined Huggingface cache: {resolved_paths[f"HF_HOME"]}")        
        print(f"Config defined Huggingface cache: {local_paths[f"HF_HOME"]}")
    if all_exist_hf==True or all_exist_dir==True:
        print("FINAL FOUNDING: It seems all model directories were found. Nothing will be downloaded!") 
    sys.exit()
#prefix end
...
fp_model_to_load="PATH_MODEL_FramePackI2V_HY"
if LOAD_F1_MODEL:
    fp_model_to_load="PATH_MODEL_FramePack_F1_I2V_HY_20250503"
transformer = HunyuanVideoTransformer3DModelPacked.from_pretrained(resolved_paths[fp_model_to_load], torch_dtype=torch.bfloat16).cpu()
...
block = gr.Blocks(css=css, title="FramePack_core").queue()
    ...
    with gr.Row():
        with gr.Column():
            ...
            with gr.Group():
                ...
                gpu_memory_preservation = gr.Slider(label="GPU Memory Preservation(in GB). Critical for URAM Systems (MacOS)", minimum=0, maximum=128, value=init_if_uram, step=0.1, info="Set this number to at a larger value if you encounter OOM. Larger value causes slower speed. Critical on systems with unified VRAM (like MacOS): Set this option to at least the amount of RAM that your OS needs to work (aroung 8 GB) or else it will crash your system. (This option is active if you have less than 60GB VRAM free(!))", visible=not high_vram)
